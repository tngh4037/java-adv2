...
public class CommandManagerV2 implements CommandManager {
    ...
    public void execute(String totalMessage, Session session) throws IOException {
        ...
        if (totalMessage.startsWith("/join")) { // ex. /join|{username}
            String[] split = totalMessage.split(DELIMITER);
            String username = split[1];
            session.setUsername(username);

            sessionManager.sendAll(username + "님이 입장했습니다.");
        } else if (totalMessage.startsWith("/message")) { // ex. /message|{contents}
            // 클라이언트 전체에게 메시지 보내기
            String[] split = totalMessage.split(DELIMITER);
            String message = split[1];

            sessionManager.sendAll("[" + session.getUsername() + "] " + message);
        } else if (totalMessage.startsWith("/change")) { // ex. /change|{username}
            String[] split = totalMessage.split(DELIMITER);
            String changeName = split[1];
            sessionManager.sendAll(session.getUsername() + "님이 " + changeName + "로 이름을 변경했습니다.");

            session.setUsername(changeName);
        } else if (totalMessage.startsWith("/users")) { // ex. /users
        ...
        } else if (totalMessage.startsWith("/exit")) {  // ex. /exit
