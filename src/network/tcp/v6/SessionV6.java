...
public class SessionV6 implements Runnable {
    ...
    public synchronized void close() {
        if (closed) { // 동시성 처리를 통해 동시에 호출되는 것을 막더라도, 이후에 호출되는 것은 자원을 중복해서 닫으면 안되므로 상태체크를 해주었다. ( 중복 호출 되는 것을 막기 위해 ) | 참고) 사실 중복 호출되어도 크게 문제가 되진 않는다. -> 내부에서 (이미 close된 경우는 닫지않는) 예방 코드가 들어가 있다.
            return;
        }
